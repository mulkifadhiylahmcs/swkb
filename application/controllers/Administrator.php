...
class Administrator extends CI_Controller {
	...
	function detail_pesanmasuk(){
		...
		if (isset($_POST['submit'])){
            ...
            $this->email->from('mulki.fadhiylah.mcs@gmail.com', 'GATRA DEV MAIL');
            $this->email->to($email);
            $this->email->cc('');
            $this->email->bcc('');
    ...
    function upgrade(){
		...
		echo $this->session->set_flashdata('message', '<div class="alert alert-success"><center>Dashboard SWKB Berhasil diperbaharui, Terima kasih..</center></div>');
		redirect($this->uri->segment(1).'/home');
	}
